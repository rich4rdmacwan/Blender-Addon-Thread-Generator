bl_info = {
    ...
    "author": "Richard Macwan, forked from Alfonso Annarumma, http://otvinta.com/",
    "version": (1, 1),
    "blender": (2, 80, 0),
...
class OBJECT_OT_add_thread(Operator, AddObjectHelper):
    ...
    def draw(self, context):
        ...
        #row = layout.row()
        #row.template_icon_view(context.scene, "my_thumbnails", scale=20)
        #row = layout.row()
        #row.label(text="Coefficients guide")
        ...
        col.template_icon_view(context.scene, "my_thumbnails", scale=15)
        #col.template_palette(context.scene,"my_thumbnails")
        #my_icon = pcoll["my_icon"]
        #row.operator("render.render", icon_value=my_icon.icon_id)
        #row.operator("render.render", text="My text", icon = "SCENE")
...
def register():
    ...
    pcoll.images_location = bpy.path.abspath(os.path.join(os.path.dirname(__file__), "icons"))
    #pcoll.images_location = os.path.join(os.path.dirname(__file__), "icons")
    ...
    annotation_image_path = os.path.join(os.path.dirname(__file__), "icons\\threadcalc_annot.png")
